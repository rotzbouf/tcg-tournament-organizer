...
### Added
- **Commander-Pods: Multiplayer-Turniere als neues Format** — Turniere mit 4-Spieler-Pods statt Zweier-Matches, gedacht für Commander/cEDH (funktioniert für jedes Spiel). Die Paarung folgt dem verbreiteten TopDeck-Standard: Pods top-to-bottom nach Punkten, Wiederholungs-Podmates werden über alle Runden hinweg vermieden, und statt Freilosen entstehen am Tabellenende 3er-Pods (nur bei genau 5 Spielern ein 5er-Pod) — niemand sitzt eine Runde aus. Die Sitzreihenfolge wird pro Pod zufällig gelost, gespeichert und überall als Zugreihenfolge angezeigt (Desktop, Handy, Druck). Punkte wählbar beim Erstellen: 5 pro Sieg und 1 pro Unentschieden (TopDeck-Standard) oder 7 pro Sieg (2n−1 nach dem Judge-Addendum); Tiebreaker: Punkte → eigene Match-Win-Rate → durchschnittliche Gegner-Punkte → Gegner-Match-Win-Rate, mit eigenen Spalten in der Rangliste. Der Top Cut kommt in den zwei üblichen Größen: Top 4 als direkter Final-Pod oder Top 16 als vier Snake-geseedete Pods (1/8/9/16 usw.), deren Sieger in den Final-Pod aufsteigen; im Cut gibt es kein Unentschieden. Ergebnis-Eingabe per Antippen des Siegers in der neuen Pod-Karte (oder Pod-Unentschieden), Rundenzahl-Empfehlung passend zum Format (drei Gegner pro Runde brauchen weniger Runden), Standard-Rundenzeit 75 Minuten. Deck-Checks erfassen den ganzen Pod, Extra-Zeit funktioniert pro Pod-Tisch, die Spielersuche findet Pod-Teilnehmer. Pod-Turniere zählen zur Saison und landen mit Platzierung in der Spieler-Datenbank, verändern aber bewusst kein Elo (ein Multiplayer-Sieg ist keine Zweier-Wertung); Altersklassen und Sitzplatz-Tausch gibt es im Pod-Format nicht
- **Pod-Ergebnisse vom Handy** — Spieler melden den Pod-Sieger direkt von der Mobile-Seite („Ich habe gewonnen" oder Antippen eines Mitspielers, Unentschieden möglich); melden zwei Spieler unterschiedliche Sieger, erscheint beim Turnierleiter wie gewohnt der rote Widerspruchs-Hinweis mit Entscheidungs-Buttons für alle Pod-Mitglieder. Judges tragen Pod-Ergebnisse vom Handy direkt ein (ein Knopf pro Spieler, ohne Spielstand-Schritt — Pods haben keine Spielstände) mit der gewohnten namentlichen Zuordnung; die Paarungsliste auf allen Handys zeigt Pods mit Zugreihenfolge, Extra-Zeit und Sieger-Badge
- **Druck & Export pod-fähig** — Match-Slips als Pod-Zettel (eine Sieger-Box pro Sitz, Unentschieden-Box, eine Unterschriftszeile pro Spieler), Paarungslisten nach Tisch und nach Name mit Pod-Mitspielern, Ranglisten-Aushang, Abschlussbericht und Ergebnis-CSV mit den Pod-Tiebreaker-Spalten; auch die Discord-Paarungen zeigen Pods. Die Export-Brücken zu TOM/EventLink/KTS sind bei Pod-Turnieren ausgeblendet — die offiziellen Tools kennen keine Multiplayer-Pods

### Fixed
- **Import von Sicherungen mit den neuen Spielen** — Der JSON-Import lehnte Dateien ab, die Turniere in Flesh and Blood, One Piece oder Dragon Ball Super: Fusion World enthalten („invalid game type") — die interne Liste gültiger Spiele war beim Hinzufügen der drei Spiele in 1.7.0 nicht mitgewachsen. Die Liste wird jetzt aus der zentralen Spiel-Registrierung abgeleitet und kann nicht mehr veralten
...
- **Spielstand bei der Judge-Ergebniseingabe** — Judges tragen Ergebnisse vom Handy jetzt zweistufig ein: erst Sieger oder Unentschieden antippen, dann den Spielstand aus Schnellauswahlen wählen (z.B. 2-0 / 2-1 / 1-0; „ohne Spielstand" bleibt möglich, ✕ bricht ab). Die Spielstände fließen damit auch bei Judge-Eingaben in die Spiel-Tiebreaker ein, statt am Turnierleiter-Gerät nachgetragen werden zu müssen
- **Judge-Ruf mit Grund** — Spieler geben beim Judge-Ruf jetzt an, worum es geht: vier Schnellauswahlen (Regelfrage, Ergebnis-Streit, Fehlendes Material, Sonstiges) plus optionale Kurzbeschreibung. Der Grund erscheint im Hinweis-Fenster des Turnierleiters und in der Ruf-Liste auf den Judge-Handys — der Judge weiß schon auf dem Weg zum Tisch, worum es geht
- **Judge-Rufe als erledigt markierbar** — Zusätzlich zum Übernehmen lässt sich ein Ruf jetzt abschließen; das darf jeder Judge, nicht nur der Übernehmende (wer ohne Übernahme erledigt, übernimmt automatisch). Erledigte Rufe wandern gedimmt in einen aufklappbaren Abschnitt mit „erledigt von …", und der Zähler am Rufe-Tab zählt nur noch offene Rufe — vorher verschwand ein Ruf erst nach Ablauf von 15 Minuten aus dem Zähler
- **Deck-Checks vom Judge-Handy** — Der Judge am Tisch startet den Deck-Check jetzt selbst: 🔍-Knopf am laufenden Match (maximal ein Check pro Tisch und Runde), ein Banner zeigt den laufenden Check mit Startzeit und öffnet die Decklisten beider Spieler; abgeschlossen wird mit „OK — kein Befund" oder „Befund → Strafe" (springt direkt in den Strafen-Tab mit vorausgewähltem Spieler), Abbrechen ist möglich. Die offizielle Zeitgutschrift (Check-Dauer + 3 Minuten) wird wie beim Turnierleiter automatisch vergeben; auch die anderen Judge-Handys sehen den laufenden Check. Die Zufallstisch-Auswahl bleibt bewusst beim Turnierleiter
- **Nachvollziehbar, welcher Judge was eingetragen hat** — Strafen, Ergebnisse, Drops (auch per Disqualifikation) und Deck-Checks vom Judge-Handy tragen jetzt den Namen des Judges. Er erscheint mit ⚖-Symbol in der Match-Karte, der Spielerliste und dem Deck-Check-Protokoll sowie als „vergeben von …" in der Strafenliste und der spielerübergreifenden Strafen-Historie. Korrigiert der Turnierleiter ein Ergebnis am Desktop, wird die alte Zuordnung entfernt (sonst stünde der falsche Name daran). An die Spieler-Handys wird die Zuordnung eines Drops nicht übertragen — sie könnte eine Disqualifikation verraten

### Changed
- **Ein Judge-QR pro Judge statt eines geteilten Codes** — Der Turnierleiter erstellt Judge-Zugänge jetzt einzeln mit Namen („Judge-QR erstellen"); der Mobile-Tab führt eine Liste aller ausgegebenen Zugänge mit QR-erneut-Anzeigen und Widerruf — einzeln (ein Judge geht heim, die anderen arbeiten weiter) oder alle auf einmal. Der vergebene Name ist die verbindliche Kennung für die Nachvollziehbarkeit: Das Judge-Handy übernimmt ihn automatisch, es muss nichts mehr eingetippt werden, und er kann nicht durch einen am Gerät getippten Namen ersetzt werden. Bei namenlos erstellten Zugängen gilt weiterhin der am Gerät gewählte Name
...
- **Extra-Zeit pro Tisch (Time Extensions)** — Wie bei offiziellen Events kann einem einzelnen Tisch Nachspielzeit gewährt werden (Judge-Ruling, Deck-Check, verspäteter Rundenstart): Der Turnierleiter vergibt sie direkt in der Match-Karte (+1/+3/+5 Minuten, kumulierend, per ✕ wieder entfernbar; die Buttons erscheinen, sobald der Rundentimer benutzt wird), Judges vom Handy aus über die Judge-Ansicht. Das Match zeigt die gewährte Zeit als Badge; läuft der Rundentimer ab, zählt am verlängerten Tisch ein eigener Countdown sichtbar weiter, während die übrigen Tische „Zeit abgelaufen" zeigen. Auch die Paarungsliste auf den Spieler-Handys zeigt die Extra-Zeit an
- **Deck-Check-Workflow** — Im Runden-Tab wählt ein Klick auf „Deck-Check (Zufallstisch)" einen zufälligen Tisch mit noch laufendem Match aus (maximal ein Check pro Tisch und Runde). Ein Banner zeigt den laufenden Check mit Uhr und öffnet auf Wunsch die Decklisten beider Spieler; abgeschlossen wird mit „OK — kein Befund" oder „Befund → Strafe" (öffnet direkt den Strafen-Dialog mit vorausgewähltem Spieler), Abbrechen ist möglich. Beim Abschluss erhält der Tisch automatisch die offizielle Zeitgutschrift: Dauer des Checks plus 3 Minuten. Alle Checks landen in einem aufklappbaren Protokoll (Runde, Tisch, Spieler, Uhrzeit, Ergebnis); das Protokoll bleibt auf dem TO-Gerät und wird nicht an die Handys übertragen
- **Offizielle Top-Cut-Größen und Rundenzahlen pro Spiel** — Beim Erstellen und Bearbeiten eines Turniers mit Top Cut gibt es jetzt eine echte Cut-Auswahl: „Automatisch (offizielle Größe)" oder fest Top 4/8/16/32. Automatisch richtet sich nach den offiziellen Tabellen des jeweiligen Spiels (Magic: MTR Anhang E — ab 9 Spielern Top 8; Pokémon: Play!-Handbuch — 9–20 Top 4, ab 21 Top 8; Yu-Gi-Oh!: Konami Policy 2.5 — 9–32 Top 4, ab 33 Top 8; übrige Spiele üblich 9–16 Top 4, ab 17 Top 8) statt der bisherigen generischen Staffel bis Top 32. Ein Hinweis im Dialog nennt die Regel des Spiels; wählt der Turnierleiter bewusst etwas anderes, erscheint dezent „Offiziell für N Spieler: Top X". Zusätzlich folgt die Swiss-Rundenzahl jetzt den offiziellen Mit-Cut-Strukturen, die in einigen Bereichen von der reinen Spielerzahl-Formel abweichen (z.B. Magic und Pokémon mit Cut: 9–16 bzw. 13–16 Spieler → 5 Runden statt 4)

### Changed
- **Großturniere deutlich schneller** — Bei 512 Spielern brauchte die Ranglisten-Berechnung bis zu ~1 Sekunde und die Rundengenerierung bis zu ~2,7 Sekunden; jetzt sind es wenige Millisekunden bzw. maximal ~0,2 Sekunden. Die Rangliste wird in einem einzigen Durchlauf über alle Matches berechnet statt pro Spieler und Gegner erneut, und die Paarungssuche betrachtet bei großen Feldern nur noch Kandidaten nahe der eigenen Punktzahl — mit automatischem Rückfall auf die vollständige Suche, falls das je einen Spieler unpaarbar machen oder ein Rematch erzwingen würde. Die Ergebnisse sind nachweislich identisch zur bisherigen Berechnung (feldgenauer Vergleichstest über ein komplettes 512-Spieler-Turnier); Turniere bis 128 Spieler paaren exakt wie zuvor
